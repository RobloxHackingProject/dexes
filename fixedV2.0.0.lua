--[[
Credits to the original makers of V2.0.0.
Fix made by RobloxHackingProject.
This fix fixes the ClickPartToSelect and the SelectionBox function.
]]
...
-- Returns the first element of the selection, if there is one
local function getSelection()
    local selection = game:GetService("CoreGui").Dex.ExplorerPanel.TotallyNotGetSelection:Invoke()
...
-- Selects the clicked object if the explorer window is open and ClickSelect is true
Mouse.Button1Down:connect(function()
    if CurrentWindow == "Explorer" and Settings.ClickSelect then
        local target = Mouse.Target
        if target then
            game:GetService("CoreGui").Dex.ExplorerPanel.TotallyNotSetSelection:Invoke({target})
        end
    end
end)

-- Updates the selection box to highlight the selected object, if SelBox is true
TotallyNotSelectionChanged.Event:connect(function()
	...
	local Selection do
		...
		local bindSetSelection = explorerPanel:WaitForChild("TotallyNotSetSelection")
